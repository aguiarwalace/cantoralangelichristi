...
/* =======================================================
   CARGA Y FILTRADO DINÁMICO DEL CALENDARIO (CON UBIC_URL)
   ======================================================= */
...
async function cargarEventosDinamicos() {
    ...
    try {
        const response = await fetch(URL_WEB_APP_GOOGLE);
        ...
        // 2. Filtrar solo eventos de hoy en adelante
        ...
        if (proximos10Eventos.length === 0) {
            eventsContainer.innerHTML = "<p>No hay eventos próximos programados.</p>";
            return;
        }

        // 4. Construir el HTML
        eventsContainer.innerHTML = proximos10Eventos.map(ev => {
            ...
            // Línea 2: Verificar si existe enlace en 'ubic_url'
            let linkPin = '';
